...
4. 编写摘要 + 每章正文（中英双语 `data-lang="zh"` + `data-lang="en"`）
...
## 人类化写作指令（Humanization）

生成的报告正文必须去 AI 味。以下规则基于主流 AIGC 检测系统的 5 个判定维度（D1-D5）：

### D1-句长分布
- 禁止连续 3 句长度差 < 8 字
- 每段必须混入 ≤10 字的短句 1-2 句，以及 ≥35 字的长句 1-2 句
- 段落不以相同的句式开头（如连续两段以 "X 是 Y" 或 "根据..." 开头）

### D2-段落结构
- 相邻段落不得使用同一种结构模板。从以下模板轮换：
  · 数据驱动：抛数字 → 阐释 → 引申
  · 对比判断：呈现 A → 呈现 B → 分析差异
  · 问题先行：提问题 → 逐层分析 → 给结论
  · 观察收束：深入论证 → 直接判断 → 不附加总结句
  · 叙事式：场景描写 → 自然引出要点
- 段落不以相同的句式开头（如连续两段以 "X 是 Y" 或 "根据..." 开头）

### D3-信息密度
- 核心论点段：密度 70%-85%，每个主张配具体数据
- 过渡句/段：密度 40%-50%，1-2 句即可
- 连续两段密度差 ≥ 15%，形成"高-低-高"交替

### D4-连接词控制
- 禁止以下 AI 高频连接词出现在段落开头：首先/其次/最后 | 综上所述 | 此外/另外 | 值得注意的是/需要指出的是
- 每 1000 字连接词 ≤ 6 个，仅在逻辑转折处集中出现
- 替代方式：用上一段的结论自然引出下一段的问题

### D5-术语变体
- 每 800 字至少 1 处用同义学术表达替代标准术语
  · "重要趋势" → "绕不开的方向" / "核心方向" / "关键走向"
  · "增长驱动" → "拉动因素" / "推力" / "支撑逻辑"
  · "占据主导" → "盘踞顶端" / "稳坐头把交椅"
  · "优势明显" → "强一截" / "拉开差距" / "高出不止一个层次"
- 使用不常见但准确的表达，不滥用

### 英文文本
- 英文句长同样要变化：混入驻句（2-6 词）和复合句（30+ 词）
- 避免每句都以 "The"/"This"/"These" 开头
- 连接词同样降密度：删掉 "Furthermore"/"Moreover"/"Notably" 等
