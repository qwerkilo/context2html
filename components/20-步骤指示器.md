...
当需要在报告中引导读者了解流程进度时，用水平步骤条替代"第一步/第二步/第三步"的文字表述。已完成步骤用强调色实心圆，当前步骤用描边高亮，待办步骤灰色。
...
.si-step.current .si-num { background: var(--bg); color: var(--accent); border-color: var(--accent); box-shadow: 0 0 0 4px color-mix(in srgb, var(--accent) 20%, transparent); }
