...
.sc-dot { width: 28px; height: 28px; border-radius: 50%; margin-bottom: 0.4em; position: relative; background: var(--bg); transition: all var(--anim-dur, 0.2s) ease; }
.sc-step.done .sc-dot { background: var(--accent); border: 3px solid var(--accent); }
.sc-step.current .sc-dot { background: var(--bg); border: 3px solid var(--accent); box-shadow: 0 0 0 4px color-mix(in srgb, var(--accent) 20%, transparent); }
