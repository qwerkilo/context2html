...
**每章必须使用**，放在每章结尾。当需要展示一组关键词、分类标签或技术栈时，用彩色药丸标签替代逗号分隔的文本列表。5 种预定义颜色对应不同类别。
...
.tag { display: inline-block; padding: 0.25em 0.7em; border-radius: 999px; font-size: 0.8rem; font-weight: 500; white-space: nowrap; background: var(--tag-bg); color: var(--tag-text); }
