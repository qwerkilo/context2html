...
description: 调研报告场景下的视觉组件选择矩阵、CSS 变量参考与快速决策
---
...
需要展示什么?
├── 数据/数字
│   ├── 3D 多维数据 → Three.js 3D 柱状图 #26
│   ├── 柱状/饼图/折线 → ECharts #24
│   ├── 地理/矩阵密度 → 热力图 #19
│   ├── 力导向/桑基/旭日 → D3 力导向图 #27
...
| 报告内容类型 | 推荐组件 | 优先级 | 失败模式 / 何时不用 |
|------------|---------|--------|-------------------|
| 数据对比（柱状图/折线图/饼图） | ECharts #24 | ⭐⭐⭐ | 数据点<3 时用 CSS 条形图 #4 更轻量；饼图>8 类别改条形图；移动端响应式需设置 `width:100%` |
| 3D 数据分布 | Three.js 3D 柱状图 #26 | ⭐⭐⭐ | 维度≤2 时 ECharts 2D 更清晰；移动端性能敏感不适用；WebGL 不支持时无降级方案 |
| 3D 地理分布 | ECharts GL 3D 地球 #28 | ⭐⭐⭐ | 数据与地理无关时不适用；仅 1-2 个点数据时过重；浏览器不支持 WebGL 时需降级为 2D 地图 |
| 3D 散点/聚类 | ECharts GL 3D 散点图 #28 | ⭐⭐ | 数据点<20 时统计意义不足；无 GL 支持时用 2D 散点图 |
| 3D 趋势曲面 | ECharts GL 3D 曲面 #28 | ⭐⭐ | 数据非连续曲面时不适用；建议同时提供 2D 热力图 #19 作为降级 |
| 3D 关系网络 | Three.js + D3 组合 #26+#27 | ⭐⭐ | 节点<15 时 D3 力导向图 #27 足够；注意同时加载两个库的心理模型成本 |
| 3D 产品展示 | Three.js 3D 场景 #26 | ⭐⭐ | 只需图标/平面展示时不适用；需考虑加载时间（Three.js ~500KB） |
| 3D 堆叠对比 | ECharts GL 3D 柱状图 #28 | ⭐⭐ | 对比维度≤2 时 ECharts 2D 更直观；GL 库额外 300KB |
| 数值高低分布 | 热力图 #19 | ⭐⭐ | 矩阵<3×3 时信息量不足；纯 CSS 实现，无需外部依赖 |
| 流程/架构说明 | SVG 流程图 #1 | ⭐⭐⭐ | 步骤≤3 时纯文本即可；SVG 文件需检查 XML 合法性 |
| 时间线事件 | CSS 时间线 #3 / 交互式时间线 #11 | ⭐⭐ | 事件≤2 时不适用；#11 需 JS 支持，降级为纯 CSS #3 |
| 关键指标 | 数据卡片网格 #12 / 数值滚动 #16 | ⭐⭐⭐ | 指标≤1 时不适用；滚动动画 #16 在 `prefers-reduced-motion` 时自动静音 |
| 多维度对比 | 对比表增强版 #22 | ⭐⭐ | 对比项≤2 时普通列表即可；粘性表头在窄屏可能遮挡内容 |
| 来源引用 | 引文卡片 #13 | ⭐⭐ | 无需突出引用时不使用；过多引文卡片会降低正文可读性 |
| 复杂概念分步 | 折叠分步 #8 / Tab 面板 #9 | ⭐⭐ | 内容无需分步/分 Tab 时不适用；#8 所有折叠默认展开 |
| 关系网络 | D3 力导向图 #27 | ⭐ | 节点<5 时信息量不足；D3 ~300KB，轻量场景用 CSS 简化 |
| 状态/进度 | 状态链 #15 / 步骤指示器 #20 | ⭐ | 步骤≤2 时不适用；纯 CSS 零依赖 |
| 提示/警告 | 告警条 #18 | ⭐ | 无需风险提示时不使用；4 种类型（info/warning/error/success） |
| 补充信息 | 信息面板 #21 | ⭐ | 无需补充内容时不使用；窄屏需测试右侧抽屉是否遮挡正文 |
| 标签/关键词 | 标签组 #17 | ⚠️ 强制 | 每章结尾必须插入；5 色自动循环 |
| 图片说明 | SVG Figure #6 / 标注式图片 #14 | ⭐⭐ | 无需图片时不使用；#14 的标注点>8 个时视觉过载 |

## 使用规则

- **每 500 字至少 1 个视觉元素**，数据密集型章节可缩短到 300 字
- **数据密集型报告优先使用 Three.js #26 和 ECharts GL #28** 提升视觉冲击力
