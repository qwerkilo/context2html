...
|------------|---------|--------|-------------------|
| 数据对比（交互式 ECharts） | ECharts #26 | ⭐⭐⭐ | 数据点<3 时用 CSS 条形图 #4 更轻量；饼图>8 类别改条形图；需 `libs/echarts.min.js` |
| 数据对比（基础/轻量） | CSS 条形图 #4 / 数据图表集 #24 | ⭐⭐ | 零 JS 依赖；#24 含柱状/饼图/折线/堆叠；#4 适合水平对比 |
| 3D 数据分布 | Three.js 3D 柱状图 #27 | ⭐⭐⭐ | 维度≤2 时 ECharts 2D 更清晰；移动端性能敏感不适用；WebGL 不支持时无降级方案 |
| 3D 地理分布 | ECharts GL 3D 地球 #29 | ⭐⭐⭐ | 数据与地理无关时不适用；仅 1-2 个点数据时过重；浏览器不支持 WebGL 时需降级为 2D 地图 |
| 3D 散点/聚类 | ECharts GL 3D 散点图 #29 | ⭐⭐ | 数据点<20 时统计意义不足；无 GL 支持时用 2D 散点图 |
| 3D 趋势曲面 | ECharts GL 3D 曲面 #29 | ⭐⭐ | 数据非连续曲面时不适用；建议同时提供 2D 热力图 #19 作为降级 |
| 3D 关系网络 | Three.js + D3 组合 #27+#28 | ⭐⭐ | 节点<15 时 D3 力导向图 #28 足够；注意同时加载两个库的心理模型成本 |
| 3D 产品展示 | Three.js 3D 场景 #27 | ⭐⭐ | 只需图标/平面展示时不适用；需考虑加载时间（Three.js ~500KB） |
| 3D 堆叠对比 | ECharts GL 3D 柱状图 #29 | ⭐⭐ | 对比维度≤2 时 ECharts 2D 更直观；GL 库额外 300KB |
| 人物/角色介绍 | 角色卡片 #2 | ⭐⭐ | 无人物介绍时不使用；网格布局自动适应 |
| 图片前后对比 | 图片对比滑块 #10 | ⭐⭐ | 需两张同尺寸图片；JS 拖拽交互 |
...
| 关系网络 | D3 力导向图 #28 | ⭐ | 节点<5 时信息量不足；D3 ~300KB，轻量场景用 CSS 简化 |
| 状态/进度 | 状态链 #15 / 步骤指示器 #20 | ⭐ | 步骤≤2 时不适用；纯 CSS 零依赖 |
| 进度环/进度仪表 | SVG.js #31d | ⭐⭐ | 单指标可视化；进度<3 时不适用；需 libs/svg.min.js |
