#!/usr/bin/env python3
"""
generate-theme-css.py — 从 teach_more_pic theme/DESIGN.md 读取 YAML 设计 Token，
生成报告专用的 theme CSS 文件和主题索引参考。
...
# 查找 teach_more_pic 主题目录（同级或上级）
TEACH_MORE_PIC_PATHS = [
    os.path.join(os.path.dirname(PROJECT_DIR), "teach_more_pic", "theme"),
    os.path.join(os.path.abspath(os.sep), "Users", "qwerkilo", ".agents", "skills", "teach_more_pic", "theme"),
    os.path.join(os.path.abspath(os.sep), "home", "qwerkilo", ".agents", "skills", "teach_more_pic", "theme"),
    os.path.join(os.environ.get("HOME", ""), ".agents", "skills", "teach_more_pic", "theme"),
]

THEME_DIR = None
for p in TEACH_MORE_PIC_PATHS:
    if os.path.isdir(p):
        THEME_DIR = p
        break

if not THEME_DIR:
    print("[WARN] teach_more_pic theme dir not found; trying relative CWD")
    THEME_DIR = os.path.abspath("theme")
    if not os.path.isdir(THEME_DIR):
        print("[ERROR] Cannot find theme directory. Set TEACH_MORE_PIC_THEME env or symlink.")
        sys.exit(1)
...
/* Do not edit directly — edit theme/DESIGN.md and regenerate */
...
def generate_markdown_index(themes):
    ...
    lines = [
        '# context2html — 主题索引\n',
        f'共 {len(themes)} 个主题，由 `scripts/generate-theme-css.py` 从 teach_more_pic theme/DESIGN.md 自动生成。\n',
        '## 主题一览\n',
